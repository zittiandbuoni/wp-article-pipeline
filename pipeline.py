...
INTENT_CONFIG = {
    ...
    "review": {
        "label": "評判・レビュー系",
        "structure": "導入（250字）→ 基本情報（H2）→ メリット（H2）→ デメリット・注意点（H2）→ こんな人におすすめ（H2）→ まとめ＋CTA（200字）",
        "extras": (
            "- メリット・デメリットを公平に書くこと（ネガティブ評価も信頼性のために必要）\n"
            "- キーワードを必ずタイトルの先頭または前半に配置すること\n"
            "- 料金・割引率・就職率などの具体的な数字は「公式サイトによると」「調査時点では」を必ず付けること\n"
            "- メールアドレス・電話番号などの具体的な連絡先は絶対に記載しないこと\n"
            "- 導入文を含め全文で、自分が実際に体験・受講したような表現は一切使わないこと\n"
            "- まとめの末尾に以下のCTAセクションを必ず追加すること：\n"
            "  ## まず無料相談で自分に合うか確認しよう\n"
            "  （スクール名）は無料カウンセリングを実施しています。料金・カリキュラム・転職サポートについて、プロに直接相談できます。申し込みは公式サイトから数分で完了します。\n"
            "  ※リンクは後から挿入するためプレースホルダー[公式サイトURL]を記載してください"
        ),
        "faq_count": 3,
    },
    "concept": {
...
- 自分が実際に体験・利用・受講したという表現は導入文も含め一切使わないこと。「知人のエンジニアが」「口コミによると」「調査した結果」などの表現を使うこと
...
def process_keywords(start: int = 0, end: int = 10, post_to_wp: bool = True):
    ...
    for i, row in enumerate(target, start=start + 1):
        ...
        try:
            ...
            err = str(e)
            if any(w in err for w in ["credit", "billing", "insufficient", "quota", "overloaded"]):
                print(f"  → クレジット切れまたはAPI制限。処理を停止します。\n")
                break
            print(f"  → エラー: {e}\n")
            continue
